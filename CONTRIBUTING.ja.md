...
- 葉は単位を持ちません。単位は値が持ち、どれなのかは設定の一覧が言います。`poll` は `12s`、
  `width` はブロックの数です。
...
見つかれば起動を断ります。書き換える前と後の、どちらの移行をデータベースが持っているのかが
...
インデックスを有効と言ってから記録します。落ちた実行や途中で切れた実行が残すのは、同じ名前の
有効でないインデックスです。
...
エラーの文は CI の記録に残ります。コマンドが取るフラグを言い、余分な語は数で挙げてください。
...
- データベースが要るテストは `postgrestest.Fresh` で自分専用のデータベースを取ってください。
