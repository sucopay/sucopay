...
| `networks.<name>.finality.wait` | `2h` | 期限を過ぎた payment が着金を待つ長さ。`1m` 以上 |
...
`finality` の 3 つは、チェーンに載った送金をいつ払われたと認めるかを決めます。

`wait` は、期限を過ぎた payment が着金を待つ長さです。期限の直前に署名された送金は、期限の
あとにチェーンへ載ります。既定の 2 時間は、計測したエンドポイント間の確定の遅れの最大 1509 ブロック
より十分長く取ってあります。短くすると、払われた payment が期限切れになります。
