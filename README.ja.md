...
> **開発初期です。** 送金が見つかり、突き合わされ、記録され、確定し、支払いが `succeeded` に
> 届きます。支払いが `succeeded` に届くまでの道筋は、プロセスの中のチェーンに対しては
> 端から端まで、Polygon に対しては実際の支払い 1 件で 1 段ずつ確かめました。今できることと
> できないことは [ROADMAP.ja.md](ROADMAP.ja.md) にあります。
...
問い合わせます。コントラクトが拒むアドレスは断ります。残りは
