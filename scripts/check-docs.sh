...
forbidden='印字(?!可能)|訊|配備|宛先|経路|要求|応答|契約|配送|要り|要る|断り|断る|名指|押さえ|落ち(た|る|て|れ)|控え|素の|周(?=[がをはにのとでも、。]|$)|[0-9] ?周|(サーバ|ユーザ|インタフェース|アダプタ|エクスプローラ|プロバイダ)(?!ー)|(?<![A-Za-z/._])(Payment|Refund)s?(?![A-Za-z_])|(?<!API )(?<!RPC )(?<!Webhook )(?<![A-Za-z])エンドポイント'
