...
func serving(t *testing.T, h http.Handler) *api.Server {
	t.Helper()
	s, err := api.Listen("127.0.0.1:0", h, quiet())
	if err != nil {
...
func TestListen_ReportsAPortThatIsAlreadyTaken(t *testing.T) {
	...
	_, err = api.Listen(held.Addr().String(), api.Handler(quiet(), api.Dependencies{}), quiet())
...
func TestListen_AddrCarriesTheChosenPort(t *testing.T) {
	t.Parallel()
	s, err := api.Listen("127.0.0.1:0", api.Handler(quiet(), api.Dependencies{}), quiet())
	if err != nil {
		t.Fatalf("listen: %v", err)
	}
...
func TestServer_LetsAnInFlightRequestFinishAfterCancellation(t *testing.T) {
	...
	s, err := api.Listen("127.0.0.1:0", slow, quiet())
	if err != nil {
		t.Fatalf("listen: %v", err)
	}
	ctx, cancel := context.WithCancel(t.Context())
...
func TestClose_ReleasesThePort(t *testing.T) {
	...
	again, err := api.Listen(addr, api.Handler(quiet(), api.Dependencies{}), quiet())
	if err != nil {
